...
def from_answer_to_dicts(answers: List, tokenizer) -> Tuple[Dict, Dict, Dict]:
    ...
    for answer in answers:
        from_answer_to_index[answer] = \
            tokenizer.encode(answer, add_special_tokens=False)[0]
    
    from_index_to_answer = {from_answer_to_index[word]: word
                            for word in from_answer_to_index}
    ground_truth_dict = {idx:list(from_index_to_answer.keys()).index(idx)
                         for idx in from_index_to_answer}

    return from_answer_to_index, from_index_to_answer, ground_truth_dict


def convert_examples_to_features(df, tokenizer, max_input_length, **kwargs):
    """
